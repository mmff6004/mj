...
export const Gallery: React.FC<GalleryProps> = ({ images, onImageSelect }) => {
  ...
  return (
    <div className="mt-12 pt-8 border-t-2 border-gray-800">
      <h2 className="text-2xl sm:text-3xl font-bold text-center mb-6 text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-yellow-400">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {images.map((item) => (
          <div 
            key={item.id}
            className="relative group aspect-square cursor-pointer overflow-hidden rounded-lg shadow-lg"
            onClick={() => onImageSelect(item)}
            role="button"
            tabIndex={0}
            aria-label="Select image to continue editing"
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onImageSelect(item); }}
          >
            <img
              src={`data:image/png;base64,${item.editedImageBase64}`}
              alt="A generated image in the gallery"
            ...
            <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              ...
              <span className="text-white text-sm font-semibold text-center px-1">Continue Editing</span>
            </div>
          </div>
        ))}
      </div>
